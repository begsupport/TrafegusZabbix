CREATE OR REPLACE FUNCTION public.beg_fnc_valida_parametros_hot( )
    RETURNS VARCHAR
    LANGUAGE 'plpgsql'
	COST 100
    VOLATILE STRICT 
...
select name,trim(setting) as valor,pending_restart from pg_settings where name in ('log_lock_waits','huge_pages','work_mem','autovacuum','autovacuum_max_workers','idle_in_transaction_session_timeout','wal_keep_segments','fsync','maintenance_work_mem','statement_timeout','default_tablespace','shared_buffers');
...
FOR R_PARAMETRO IN C_PARAMETROS LOOP
	...
	IF R_PARAMETRO.name='shared_buffers' and R_PARAMETRO.valor <> '524288' THEN
	   V_RETORNO := V_RETORNO ||',shared_buffers = '||R_PARAMETRO.valor;
	END IF;
